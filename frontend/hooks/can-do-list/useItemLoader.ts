...
import { fetchCanDoItems, fetchCanDoItemsByProject } from '../../app/dashboard/can-do-list/actions';
import { 
  decryptData, 
...
export const useItemLoader = (
  ...
  const decryptItems = useCallback((encryptedItems: any[], key: string): CanDoItem[] => {
  ...
  const loadItems = useCallback(async (key: string): Promise<CanDoItem[]> => {
    try {
      ...
      const decryptedItems = decryptItems(encryptedItems, key);
  ...
  }, [itemActions, setError, decryptItems]);

  const loadItemsByProject = useCallback(async (key: string, projectId?: string): Promise<CanDoItem[]> => {
    try {
      itemActions.setIsLoading(true);
      const encryptedItems = await fetchCanDoItemsByProject(projectId);
      const decryptedItems = decryptItems(encryptedItems, key);
      ...
      console.error('Error loading items by project:', error);
      setError('Failed to load your Can-Do List items');
      itemActions.setIsLoading(false);
      return [];
    }
